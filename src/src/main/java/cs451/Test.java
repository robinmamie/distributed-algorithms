...
public class Test {
    public static void main(String[] args) throws InterruptedException, UnknownHostException {
        ...
        for (int i = 0; i < nLinks; ++i) {
            links[i] = Link.getLink(11000+i);
            links[i].addListener((m, a, p) -> { 
                System.out.println((System.currentTimeMillis() - baseTime) + ": " + m);
            });
        }
        for (int i = 0; i < 100000; ++i) {
            links[0].send(new Message(1, i+1), InetAddress.getByName("127.0.0.1"), 11001);
            System.out.println((System.currentTimeMillis() - baseTime) + ": sent message " + i);
            links[1].send(new Message(2, i+1), InetAddress.getByName("127.0.0.1"), 11000);
        }
